...
## ✍️ Titres de page — texte blanc (Outfit) + accent italique (12/07, unifié le 09/09)

Grep `font-display-accent` : 7 occurrences (`AdminPage.tsx` ×2, `CataloguePage.tsx`, `ProfilePage.tsx` ×2 — hero + accent "Parcours" ajouté le 07/09 —, `ProjectDetailPage.tsx`, `NotFoundPage.tsx`) — toutes vérifiées individuellement.
...
**Taille unifiée — titre et accent identiques au pixel près, sur toutes les pages (09/09)** : jusqu'ici le dashboard admin (`AdminPage.tsx`, `TabHeader` + le h1 dédié "Mon catalogue Projets") faisait volontairement exception, l'accent y dépassant le titre (`text-5xl md:text-6xl` vs `text-4xl md:text-5xl`). Sur demande explicite ("plus de différenciation de px entre les deux, appliquer le même style sur toutes les pages"), cette exception est supprimée : **`text-4xl md:text-6xl` devient la seule échelle** pour tout titre porteur d'un accent italique — `ProfilePage` (hero), `CataloguePage` (hero), `ProjectDetailPage` (titre projet), `AdminPage` (`TabHeader`, réutilisé par les 6 onglets, + le h1 dédié de l'onglet Projets) partagent désormais exactement la même paire de classes sur le titre ET sur l'accent, vérifié en direct (`getComputedStyle().fontSize`, 36px mobile / 60px desktop des deux côtés partout). Palier mobile réduit au passage (`text-5xl`→`text-4xl`, 48px→36px, sur demande "réduire légèrement la taille des titres en mobile") — palier desktop inchangé (`md:text-6xl`, déjà la valeur cible pour les pages visiteur, l'admin y est monté). **Précisé le 09/09 (retour immédiat)** : la réduction ne devait viser que le mobile, pas aussi la tablette — `text-4xl` seul couvrait tout `< md` (768px), tablette comprise. Palier intermédiaire ajouté : `text-4xl sm:text-5xl md:text-6xl` (36px `< 640px` / 48px 640–767px, valeur d'origine restaurée pour la tablette / 60px `≥ 768px`), sur les 5 paires titre+accent (`ProfilePage`, `CataloguePage`, `ProjectDetailPage`, `AdminPage` `TabHeader` + le h1 dédié "Mon catalogue Projets"). `NotFoundPage.tsx` (accent inline sans classe de taille propre, hérite du parent) non concerné, pas de titre+accent pairés au sens de cette règle.

---

## 🧭 Amélioration navigation (09/09)

**Fil d'Ariane (`src/components/Breadcrumb.tsx`, nouveau)** : sur demande explicite ("faire un fil d'Ariane sous la navbar"). Composant maison (pas le primitive shadcn `src/components/ui/breadcrumb.tsx`, code mort confirmé, tokens `text-muted-foreground` non alignés sur le design system de l'app) — icône `Home` (Lucide) pour le premier maillon plutôt que le mot "Accueil" (cohérent avec le style compact des autres icônes de nav), séparateurs `ChevronRight` 14px à 50% d'opacité, dernier maillon `aria-current="page"` non cliquable en `text-on-surface font-medium`, maillons intermédiaires `text-on-surface-variant` avec `hover:text-primary`. Posé sous le header fixe :
- `CataloguePage.tsx` : "🏠 / Projets", en première ligne de `<main>` (avant le `<header>` du titre).
- `ProjectDetailPage.tsx` : "🏠 / Projets / [titre du projet]" — le header étant `fixed` et le hero de cette page pleine largeur dès la sortie du header (pas de `<main>` avec padding-top ici, cf. section fiche projet), le fil d'Ariane est posé dans un wrapper dédié (`mt-24`, largeur `max-w-[1440px]` + `px-5 md:px-16` comme le reste du contenu) juste **avant** la `<section>` hero, qui perd son propre `mt-24` (le wrapper du fil d'Ariane fournit désormais le dégagement sous le header).
- Volontairement absent de `ProfilePage.tsx` (page racine — un fil d'Ariane n'y apporte rien, "Accueil" y est déjà la page courante) et de `AdminPage.tsx` (onglets, pas une hiérarchie de pages imbriquées — ne correspond pas à la sémantique d'un fil d'Ariane).
- **Boutons outline texte de la page Profil alignés sur le style admin (09/09)** : "Détails du projet" et "Voir plus d'expériences" utilisaient un mélange de styles ad hoc (calé sur "Se connecter" du Header pour l'un, tertiary pour l'autre) — sur demande explicite ("les boutons outlines texte de la page profil doivent avoir le même style que les boutons outline de la partie admin"), les deux reprennent désormais **exactement** le pattern répété ~10 fois dans `AdminPage.tsx` : `border-white/15`/`px-5 py-2.5`/`text-sm font-medium text-on-surface`/`hover:border-primary hover:text-primary`/`focus-visible:ring-2 ring-primary ring-offset-2`, sans transform au survol/clic (pas de `hover:scale`/`active:scale`, l'admin n'en a pas non plus sur ce pattern). Vérifié en direct (`getComputedStyle`, padding `10px 20px` = `py-2.5`).

**Bouton "Accéder au projet confidentiel" en outline violet, sans effet grossissant (09/09)** : était filled `bg-secondary-container` avec `hover:scale-105` — sur demande explicite ("ne doit pas avoir l'effet grossissant et doit être en outline violet"), passe en outline `border-secondary` (violet `#7c3aed`) avec `hover:bg-secondary/10` (tinte au survol plutôt qu'un changement de bordure façon admin, qui n'a pas de sens ici — le violet reste le violet, pas de bascule vers `primary`), texte `text-on-surface` (pas `text-secondary` : `secondary` en texte plein tombe sous le seuil AA 4.5:1 sur les fonds dark de cette section, déjà documenté plus haut pour "Détails du projet"). Plus de `hover:scale-105`/`active:scale-95`/`shadow-lg`. Même gabarit (`px-5 py-2.5`) que les autres boutons de pied d'item.

**Espace manquant entre l'accordéon et le bouton CV en mobile (09/09)** : le `gap-0` posé plus tôt dans la journée ("enlever la marge au-dessus de l'accordéon") supprimait tout écart entre la colonne éditoriale (se terminant par "Afficher le CV en pdf") et l'accordéon en dessous, en empilement mobile/tablette — sur retour explicite, remonté à `gap-8` (32px), vérifié en direct. `lg:gap-28` (écart desktop, colonnes côte à côte) inchangé.

**Titres H1 Profil + fiche projet agrandis en mobile pour remplir la largeur (09/09)** : le palier mobile `text-4xl` (36px, ajouté dans une passe précédente) laissait "Enora Le Turnier"/le titre de projet visuellement en retrait de la largeur de la card sur l'appareil de l'utilisatrice (mesuré à ~99% du conteneur en simulation 375px, mais perçu en retrait sur son propre écran) — sur demande explicite ("agrandir... pour que 'Enora Le Turnier' prenne toute la largeur"), remonté à `text-5xl` (48px, palier tablette précédent) sans palier intermédiaire (`sm:text-5xl` retiré, devenu redondant avec la base) — `md:text-6xl` desktop inchangé. **Scope volontairement limité à `ProfilePage.tsx` et `ProjectDetailPage.tsx`** (la demande citait "la page projet et profil" nommément) — `CataloguePage.tsx` et `AdminPage.tsx` (`TabHeader`) gardent le palier `text-4xl sm:text-5xl md:text-6xl` de la passe précédente, ce qui réintroduit une divergence mobile entre ces deux groupes de pages (le titre+accent restent identiques entre eux au sein de chaque page, seule la valeur mobile diffère d'un groupe à l'autre) — à surveiller si une prochaine demande souhaite ré-harmoniser.

**Bouton "Voir plus d'expériences" — deux retouches le 09/09** : 1re passe, remplace le style filled primary (`bg-primary-container`) par un outline `border-tertiary`/`text-on-surface` sur demande explicite ("en tertiary blanc"). **2e passe, retour immédiat ("dois avoir le même style que 'se connecter'")** : calé caractère pour caractère sur le bouton "Se connecter" du `Header.tsx` — `border-white/15`/`px-5 py-2`/`text-sm font-medium`/`text-on-surface`/`hover:border-primary`/`duration-[var(--duration-fast)]`, plus de `border-tertiary`, `font-bold`, `hover:scale-105`, `active:scale-95` ni `focus-visible:ring-tertiary` explicites — le focus/l'état actif retombent sur les filets de sécurité globaux de `styles.css` (`@layer base`), exactement comme "Se connecter" qui ne déclare pas non plus ces classes. Vérifié en direct : `getComputedStyle` identique sur les deux boutons (bordure, taille/graisse de police, couleur, padding, rayon).

**Bloc "Impact" en style success (09/09)** : `<Alert type="info" icon={Zap} .../>` → `type="success"` sur demande explicite — passe de la teinte tertiary (`bg-tertiary/15 border-tertiary/40 text-tertiary`) à la teinte primary/teal du variant success (`bg-primary/15 border-primary/40 text-primary`, cf. section 🔔 Système d'alertes), icône `Zap` conservée (override explicite, le rôle `alert` du variant success au lieu de `status` pour `info` convient mieux à une mise en avant de résultat).

**Opacité/flou/rayon des 3 cards numérotées unifiés (09/09, 2 passes)** : Hero (`bg-surface-container/30 backdrop-blur-sm`) et Contact (`bg-surface-container-low backdrop-blur-md`, opaque) divergeaient entre elles et de la card "Parcours" (déjà en `.glass-card` depuis le 07/09) — sur demande explicite ("l'opacité et le blur des 3 cards de la page profil doit être la même"), Hero et Contact passent toutes deux à `.glass-card` (`background: color-mix(..., 70%, transparent)` + `blur(12px)` + bordure `--border-glass`, cf. section 🎨 Fond & texte de base), leurs classes ad hoc (`border border-white/10`, `bg-surface-container*`, `backdrop-blur-*`) retirées pour éviter tout conflit de cascade avec la classe partagée. **2e passe, retour immédiat ("les arrondis... n'ont pas l'air proportionnels et égaux")** : le rayon de bord restait lui divergent (Hero/Contact `rounded-[32px]`, Parcours `rounded-2xl`, 16px) — Parcours passé à `rounded-[32px]` à son tour. Les trois cards partagent désormais exactement le même fond/flou/bordure/rayon, vérifié en direct (`getComputedStyle`, valeurs identiques). La card "Réserver un créneau" (secondaire, à l'intérieur de la section Contact) garde son propre style `bg-surface-container-low backdrop-blur-md` + `rounded-[32px]` (déjà aligné sur ce rayon, jamais changé) — pas comptée parmi "les 3 cards" mais son rayon coïncidait déjà.

**Style des maillons cliquables aligné sur les liens du site (09/09, retour immédiat)** : la couleur/le survol ad hoc (`text-on-surface-variant hover:text-primary`) est remplacée par `textLinkClass("default")` (`src/lib/linkStyles.ts`) — même style que tout autre lien texte de l'app (`font-semibold text-primary`, pas de soulignement au repos, soulignement au survol), sur demande explicite ("le fil d'ariane sont des liens, gérer leur style comme les liens"). Le dernier maillon (page courante, non cliquable) garde son propre style `text-on-surface font-medium`, hors du champ de `textLinkClass` puisque ce n'est pas un lien.
- **Position affinée (09/09, retour immédiat)** : rapproché du header (`CataloguePage` : `<main>` passé de `pt-32` à `pt-24` ; `ProjectDetailPage` : déjà `mt-24`, inchangé) — ~19px entre le bas du header et le haut du fil d'Ariane sur les deux pages, vérifié en direct (`getBoundingClientRect`). Espace explicite ajouté **après** le fil d'Ariane, avant le contenu qui suit : `CataloguePage`, le `<header>` du titre passe de `mt-6` à `mt-10` (~72px jusqu'au `<h1>`) ; `ProjectDetailPage`, le wrapper du fil d'Ariane passe de `pb-4` à `pb-8` (~32px jusqu'à la section hero, mesuré sur le bord visible `overflow-hidden` de la section, pas sur l'`<img>` qui déborde vers le haut via son recadrage `top: -40%`).

**Bouton "retour en haut" redescendu (`ScrollToTopButton.tsx`)** : sur demande explicite ("redescendre le bouton vers la fin de page"). Était centré verticalement (`top-1/2 -translate-y-1/2`, flottant au milieu de l'écran en permanence dès 400px de scroll) — passé en `bottom-6 right-5 md:bottom-8 md:right-16`, ancré en bas comme la quasi-totalité des boutons "back to top" du web. Seuil d'apparition (`scrollY > 400`) et logique opacité/`pointer-events` inchangés.

**Cards projet — étiquette/valeur empilées (`ProjectCard.tsx`)** : sur demande explicite ("mettre les labels et les infos l'un en dessous de l'autre"). Le bloc Entreprise/Rôle utilisait `flex items-center justify-between` (étiquette à gauche, valeur à droite, sur une seule ligne) — passé en `flex flex-col gap-0.5` (étiquette au-dessus, valeur en dessous), `space-y-2`→`space-y-3` sur le conteneur pour compenser visuellement la hauteur ajoutée. Le reste de la carte (titre+durée, badges de tags) n'est pas concerné — seul ce bloc avait un vrai couple étiquette/valeur.

**Scrollbar modernisée (`styles.css`, `@layer base`)** : sur demande explicite ("scrollbar à styliser de façon plus moderne"). Globale (`html`/`::-webkit-scrollbar`, Firefox + Chrome/Safari/Edge séparément, aucune propriété standard commune) — fine (10px, `scrollbar-width: thin`), thumb arrondi (`border-radius: 999px`) coloré `--outline-variant` (même token que les séparateurs de l'app) avec un liseré `--background` (`border` + `background-clip: padding-box`, donne l'impression d'un padding autour du thumb — pas de `padding` réel, non supporté sur les pseudo-éléments scrollbar), survol en `--primary`. Track transparent (pas de rail visible).
...
**Couleur des numéros de section unifiée en blanc (14/09)** : `SECTION_NUMBER_CLASSES` (page profil, `hero`/`experiences`/`contact`) et `BLOCK_NUMBER_CLASSES` (fiche projet, 3 blocs) utilisaient jusque-là 3 couleurs distinctes par section pour le numéro ET le filet dessous (`text-primary/90`+`bg-on-primary/20`, `text-secondary/90`+`bg-secondary/20`, `text-tag-keywords/90`+`bg-tag-keywords/20`) — les deux remplacés uniformément par **`text-foreground/90`** / **`bg-foreground/20`** (blanc/off-white `--on-background`, sur demande explicite, retour immédiat pour inclure aussi le filet initialement laissé inchangé).
...
**Section élargie au-delà du gabarit Hero/Contact, puis revenue en arrière (09/09)** : `lg:-mr-12` ajouté sur la `<section>` elle-même sur demande explicite ("agrandir la largeur du bloc Expériences") — la card Expériences dépassait alors de 48px, à droite uniquement, la largeur des cards Hero/Contact. **Retiré plus tard le même jour** ("il manque une marge à droite de la card Parcours pour qu'elle soit centrée") : ce bleed asymétrique (uniquement à droite, jamais à gauche, pour ne pas désaligner le numéro "02") cassait la symétrie visuelle de la card par rapport à Hero/Contact, perçue comme "non centrée" plutôt que "plus large" — `lg:-mr-12` supprimé, la card Expériences repartage désormais exactement la même largeur (bord droit compris) que Hero et Contact, vérifié en direct (`getBoundingClientRect().right` identique aux trois sections à 1440px).
