...
**Couleur des numéros de section unifiée en blanc (14/09)** : `SECTION_NUMBER_CLASSES` (page profil, `hero`/`experiences`/`contact`) et `BLOCK_NUMBER_CLASSES` (fiche projet, 3 blocs) utilisaient jusque-là 3 couleurs distinctes par section (`text-primary/90`, `text-secondary/90`, `text-tag-keywords/90`) — remplacées uniformément par **`text-foreground/90`** (blanc/off-white `--on-background`, sur demande explicite). Les couleurs `rule` (filet sous le numéro) restent inchangées et toujours différenciées par section.
