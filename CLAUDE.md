...
- **Description longue vs trio IA, mutuellement suffisants (11/07)** : `long_desc` (rédaction manuelle) et le trio Problème/Décisions/Résultat (généré par IA ou saisi à la main) racontent le projet de deux façons alternatives — la validation exige qu'au moins l'un des deux soit intégralement rempli, jamais les deux à la fois. Côté UI (`ProjectDrawer.tsx`), le champ "Description longue" se masque automatiquement dès que le trio est complet (remplacé par un lien "Rédiger quand même une description longue" réversible) — sauf si un projet existant avait déjà une description longue avant l'usage de l'IA, auquel cas elle reste visible.
