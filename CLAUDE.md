...
- **Validation** (`src/lib/projectValidation.ts`, `validateProject()`) : tous les champs scalaires obligatoires à la soumission (limites de caractères par champ avec compteur live dans le drawer — `title` 80, `short_desc` 160, `long_desc` 3000, `probleme`/`decisions`/`resultat` 500, `client_name`/`company_name` 100, `role` 60, `team` 100). Les tags (types/tools/keywords) restent volontairement optionnels.
...
**Aide IA de structuration (F-22)** — nouvelle Edge Function `generate-ai-description` (`supabase/functions/generate-ai-description/index.ts`) : prend des notes libres, appelle l'API Anthropic (`claude-sonnet-5`, sortie JSON forcée par tool-use — `tool_choice: {type: "tool", name: "structure_project"}`) et pré-remplit `probleme`/`decisions`/`resultat` + suggestions de tags (tools/keywords/types). **Vérification explicite du rôle admin côté serveur** (`get_my_role()` via un client Supabase construit avec le JWT de l'appelant) — un JWT valide (`verify_jwt: true`) authentifie mais n'autorise pas, un `pending`/`validated_visitor` authentifié pourrait sinon aussi appeler cette fonction.

**⚠️ `ANTHROPIC_API_KEY` absente des secrets Edge Functions** — même flow que `RESEND_API_KEY` précédemment (à ajouter dans Supabase Dashboard → Edge Functions → Secrets). Bloque uniquement le test end-to-end du cas de succès IA ; le cas notes vides (400) et le cas d'échec Q-02 ont été testés et confirmés en conditions réelles. Le rôle admin a été revérifié en direct avec les JWT de Sophie (pending) et Karim (validated_visitor) : INSERT/soft-delete/appel Edge Function IA tous bloqués côté serveur (403/400), pas seulement cachés côté UI.
